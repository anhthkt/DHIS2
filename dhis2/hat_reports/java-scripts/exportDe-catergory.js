...
async function processData() {
    ...
    let jsonData = await axios({
        url: 'http://dev.tkyt.vn/nhanluc/api/dataElements.json?fields=id,name,categoryCombo[categoryOptionCombos[id,name,code]],optionSet[id,name]&paging=false',
        auth: {
            username: `anhth`,
            password: `Csdl2018@)!*`
          }
    ...
    jsonData.dataElements.forEach(dataElement => {
        ...
        dataElement.categoryCombo.categoryOptionCombos.forEach(categoryOptionCombo => {
            const categoryOptionComboName = categoryOptionCombo.name;
            ...
            const columnID = `${dataElementID}-${categoryOptionComboID}-val`;
            ...
            dataArray.push({ columnName, columnID });
            wsData.addRow({
                id: columnID,
                name: columnName,
                // code: simplifiedName,
    ...
    await workbook.xlsx.writeFile('dataElement-DEV.xlsx');
    return dataArray;
